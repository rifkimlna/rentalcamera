...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() === 'mysql') {
            DB::unprepared("ALTER TABLE produk MODIFY COLUMN status ENUM('available', 'unavailable') NOT NULL DEFAULT 'available'");
        }
    }

    public function down(): void
    {
        Schema::create('promosi', function ($table) {
            $table->id();
            $table->string('judul');
            $table->string('slug')->unique();
            $table->text('deskripsi')->nullable();
            $table->string('gambar')->nullable();
            $table->enum('type', ['banner', 'popup', 'sidebar', 'email']);
        ...
        Schema::create('settings', function ($table) {
            ...
            $table->enum('type', ['string', 'text', 'number', 'boolean', 'json', 'array'])->default('string');
            $table->string('group')->default('general');
            $table->text('description')->nullable();
            $table->timestamps();
        });

        // Sintaks view & ALTER MODIFY hanya didukung MySQL
        if (DB::connection()->getDriverName() !== 'mysql') {
            return;
        }
        ...
        DB::unprepared("ALTER TABLE produk MODIFY COLUMN status ENUM('available', 'unavailable', 'maintenance') NOT NULL DEFAULT 'available'");
    }
};

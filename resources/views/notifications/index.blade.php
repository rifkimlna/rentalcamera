...
<div class="space-y-4">
    <div class="flex items-center gap-2 justify-between">
        <div class="flex gap-1">
            <a href="{{ request()->fullUrlWithQuery(['filter' => 'all']) }}" class="btn btn-sm {{ request('filter', 'all') === 'all' ? 'btn-neutral' : 'btn-outline' }}">Semua</a>
            <a href="{{ request()->fullUrlWithQuery(['filter' => 'unread']) }}" class="btn btn-sm {{ request('filter') === 'unread' ? 'btn-neutral' : 'btn-outline' }}">Belum Dibaca</a>
        </div>
        <form method="POST" action="{{ route('notifications.mark-read') }}">
            @csrf
            <button type="submit" class="btn btn-sm btn-ghost">Tandai Semua Dibaca</button>
        </form>
    </div>
            ...
            <div class="flex items-start gap-3">
                @php
                    $icons = [
                        ...
                        'shipping' => '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l2-1m4-1l2-1 2 1m-2 1v2a1 1 0 001 1h4m-3-9h3m-3 4h3"/></svg>',
                ...
                @endphp
                <div class="notification-icon bg-base-200 text-base-content/70">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            ...
                            <p class="text-xs text-base-content/40 mt-0.5">{{ $notification->message }}</p>
                        </div>
                        <span class="text-xs text-base-content/40 whitespace-nowrap">{{ $notification->created_at->diffForHumans() }}</span>
                    </div>
                    <div class="flex items-center gap-2 mt-2">
                        <span class="badge badge-outline badge-xs text-[10px]">{{ $notification->type_label }}</span>
                        @if(!$notification->is_read)
                        <span class="badge badge-xs bg-base-300 text-base-content/60 border-0">Baru</span>
                        @endif
                    </div>
                </div>
            </div>
    ...
    <div class="card bg-base-100 border border-base-300">
        <div class="card-body p-8 text-center">
            ...
            <p class="text-sm text-base-content/60">Tidak ada notifikasi</p>
        </div>
    </div>
    @endforelse

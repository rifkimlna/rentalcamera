    ...
    <div class="py-3 px-4">
        <form method="GET" class="flex flex-wrap gap-x-3 gap-y-2 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" class="input-apple input-sm w-full" placeholder="Nama user atau studio..." value="{{ request('search') }}">
            </div>
            <div>
            ...
            <div class="flex gap-1">
                ...
                <a href="{{ route('admin.studio.bookings') }}" class="btn-dark-apple !text-sm !px-3 !py-1.5">Reset</a>
            </div>
        </form>
    </div>
</div>
                    ...
                    <tr class="hover:bg-[#f5f5f7]/50">
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.studio.booking.update-status', $booking->id) }}" class="flex gap-1">
                                ...
                                @endif
                                <a href="{{ route('admin.studio.booking.print', $booking->id) }}" class="text-[#6e6e73] hover:bg-[#f5f5f7] rounded-xl !text-[10px] !px-2 !py-0.5 transition-colors" target="_blank" title="Cetak Struk">Cetak</a>
                            </form>
                        </td>
                    </tr>
                    @empty

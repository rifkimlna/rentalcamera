...
@section('title', 'Edit Voucher - Stekpro Multimedia & Broadcast')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="flex w-full">
                            <input type="text" class="input-apple w-full @error('kode_voucher') input-error @enderror"
                                   id="kode_voucher" name="kode_voucher" value="{{ old('kode_voucher', $voucher->kode_voucher) }}" required>
                    ...
                    <div>
                        ...
                        <input type="text" class="input-apple w-full @error('nama_voucher') input-error @enderror"
                               id="nama_voucher" name="nama_voucher" value="{{ old('nama_voucher', $voucher->nama_voucher) }}" required>
                        @error('nama_voucher')
                            <span class="text-[#d70015] text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select class="select-apple w-full @error('type') select-error @enderror"
                            ...
                            <option value="fixed" {{ old('type', $voucher->type) == 'fixed' ? 'selected' : '' }}>Nominal</option>
                    ...
                    <div>
                        ...
                        <input type="number" min="1" class="input-apple w-full @error('kuota') input-error @enderror"
                               id="kuota" name="kuota" value="{{ old('kuota', $voucher->kuota) }}"
                               placeholder="Kosongkan jika tanpa batas">
                        @error('kuota')
                            <span class="text-[#d70015] text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    <div>
                        ...
                        <select class="select-apple w-full" id="kategori_id" name="kategori_id">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('kategori_id', $voucher->kategori_id) == $category->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select class="select-apple w-full" id="produk_id" name="produk_id">
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" {{ old('produk_id', $voucher->produk_id) == $product->id ? 'selected' : '' }}>
                                    {{ $product->nama_produk }}
                                </option>
                            @endforeach
                        </select>

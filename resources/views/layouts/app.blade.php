...
<head>
    ...
    <meta name="description" content="@yield('meta-description', 'Stekpro Multimedia & Broadcast - Penyewaan kamera, studio foto, dan perlengkapan multimedia di Sukabumi. Harga terjangkau, alat terawat.')">
    ...
    <meta property="og:description" content="@yield('meta-description', 'Penyewaan kamera, studio foto, dan perlengkapan multimedia di Sukabumi.')">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:image" content="{{ asset('images/logo.png') }}">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:title" content="@yield('title', 'Stekpro Multimedia & Broadcast')">
    <meta name="twitter:description" content="@yield('meta-description', 'Penyewaan kamera, studio foto, dan perlengkapan multimedia di Sukabumi.')">
...
<body class="font-sans antialiased bg-white min-h-screen flex flex-col" x-data="{ mobileOpen: false }">
    ...
    <header class="fixed top-0 left-0 right-0 z-50 glass border-b border-black/5" x-data="{ scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 10 })" :class="{ 'shadow-sm': scrolled }">
        <nav class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
            <div class="flex items-center justify-between h-14 lg:h-16">
                ...
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 shrink-0">
                    <img src="{{ asset('images/logo.png') }}" alt="Stekpro Logo" class="h-9 w-auto object-contain">
                ...
                <div class="hidden lg:flex items-center">
                    <div class="flex items-center gap-1 p-1 bg-black/[0.03] rounded-full">
                        <a href="{{ route('home') }}" class="px-4 py-2 text-sm rounded-full transition-all duration-200 {{ request()->routeIs('home') ? 'bg-white text-[#1d1d1f] shadow-sm font-medium' : 'text-[#6e6e73] hover:text-[#1d1d1f] hover:bg-white/60' }}">Beranda</a>
                        <a href="{{ auth()->check() && auth()->user()->isCustomer() ? route('customer.products.index') : route('products') }}" class="px-4 py-2 text-sm rounded-full transition-all duration-200 {{ request()->routeIs(['customer.products.*', 'product.*', 'products']) ? 'bg-white text-[#1d1d1f] shadow-sm font-medium' : 'text-[#6e6e73] hover:text-[#1d1d1f] hover:bg-white/60' }}">Equipment</a>
                        <a href="{{ route('pricing') }}" class="px-4 py-2 text-sm rounded-full transition-all duration-200 {{ request()->routeIs('pricing') ? 'bg-white text-[#1d1d1f] shadow-sm font-medium' : 'text-[#6e6e73] hover:text-[#1d1d1f] hover:bg-white/60' }}">Harga</a>
                        <a href="{{ route('about') }}" class="px-4 py-2 text-sm rounded-full transition-all duration-200 {{ request()->routeIs('about') ? 'bg-white text-[#1d1d1f] shadow-sm font-medium' : 'text-[#6e6e73] hover:text-[#1d1d1f] hover:bg-white/60' }}">Tentang</a>
                        <a href="{{ route('contact') }}" class="px-4 py-2 text-sm rounded-full transition-all duration-200 {{ request()->routeIs('contact') ? 'bg-white text-[#1d1d1f] shadow-sm font-medium' : 'text-[#6e6e73] hover:text-[#1d1d1f] hover:bg-white/60' }}">Kontak</a>
                    </div>
                </div>
                ...
                <div class="hidden lg:flex items-center gap-3">
                    @auth
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                            <button @click="open = !open" class="flex items-center gap-2 px-3 py-1.5 rounded-full hover:bg-black/[0.04] transition-colors">
                                ...
                                <span class="text-sm font-medium text-[#1d1d1f] hidden xl:block">{{ auth()->user()->nama }}</span>
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95 -translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-lg border border-[#f0f0f2] py-2 z-50">
                                <div class="px-4 py-2 border-b border-[#f0f0f2] mb-1">
                                    <p class="text-sm font-medium text-[#1d1d1f] truncate">{{ auth()->user()->nama }}</p>
                                    <p class="text-xs text-[#86868b] truncate">{{ auth()->user()->email }}</p>
                                </div>
                                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-[#1d1d1f] hover:bg-[#f5f5f7] transition-colors">
                                ...
                                <a href="{{ route('profile') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-[#1d1d1f] hover:bg-[#f5f5f7] transition-colors">
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="btn-primary-apple !px-5 !py-2 !text-sm">Daftar</a>
                    @endauth
                </div>
                ...
                <div class="lg:hidden flex items-center gap-2">
                    @auth
                        <a href="{{ route('customer.cart.index') }}" class="relative p-2 -mr-1">
        ...
        <div x-show="mobileOpen" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0 -translate-y-2" class="lg:hidden glass border-t border-black/5">
            <div class="max-w-7xl mx-auto px-5 py-4 space-y-1">
                <a href="{{ route('home') }}" class="block px-4 py-2.5 text-sm rounded-xl transition-colors {{ request()->routeIs('home') ? 'bg-black/[0.04] font-medium text-[#1d1d1f]' : 'text-[#6e6e73] hover:bg-black/[0.03]' }}">Beranda</a>
                <a href="{{ auth()->check() && auth()->user()->isCustomer() ? route('customer.products.index') : route('products') }}" class="block px-4 py-2.5 text-sm rounded-xl transition-colors {{ request()->routeIs(['customer.products.*', 'product.*', 'products']) ? 'bg-black/[0.04] font-medium text-[#1d1d1f]' : 'text-[#6e6e73] hover:bg-black/[0.03]' }}">Equipment</a>
                <a href="{{ route('pricing') }}" class="block px-4 py-2.5 text-sm rounded-xl transition-colors {{ request()->routeIs('pricing') ? 'bg-black/[0.04] font-medium text-[#1d1d1f]' : 'text-[#6e6e73] hover:bg-black/[0.03]' }}">Harga</a>
                <a href="{{ route('about') }}" class="block px-4 py-2.5 text-sm rounded-xl transition-colors {{ request()->routeIs('about') ? 'bg-black/[0.04] font-medium text-[#1d1d1f]' : 'text-[#6e6e73] hover:bg-black/[0.03]' }}">Tentang</a>
                <a href="{{ route('contact') }}" class="block px-4 py-2.5 text-sm rounded-xl transition-colors {{ request()->routeIs('contact') ? 'bg-black/[0.04] font-medium text-[#1d1d1f]' : 'text-[#6e6e73] hover:bg-black/[0.03]' }}">Kontak</a>
                <div class="border-t border-[#f0f0f2] my-2"></div>
                @auth
                    <a href="{{ route('dashboard') }}" class="block px-4 py-2.5 text-sm font-medium text-[#1d1d1f] hover:bg-black/[0.03] rounded-xl">Dashboard</a>
                    <a href="{{ route('profile') }}" class="block px-4 py-2.5 text-sm text-[#6e6e73] hover:bg-black/[0.03] rounded-xl">Profil</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="block w-full text-left px-4 py-2.5 text-sm text-[#d70015] hover:bg-red-50 rounded-xl">Keluar</button>
                    </form>
                @else
                    <a href="{{ route('login') }}" class="block px-4 py-2.5 text-sm font-medium text-[#1d1d1f] hover:bg-black/[0.03] rounded-xl">Masuk</a>
                    <a href="{{ route('register') }}" class="block px-4 py-2.5 text-sm font-medium text-white bg-[#1d1d1f] hover:bg-[#333] rounded-xl text-center mt-1">Daftar</a>
                @endauth
            </div>
        </div>
    </header>
    ...
    <footer class="bg-[#f5f5f7] border-t border-[#e5e5e7]">
        <div class="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-12 lg:py-16">
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
                ...
                <div>
                    ...
                    <ul class="space-y-2.5">
                        <li><a href="{{ route('customer.products.index') }}" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] transition-colors">Equipment</a></li>
                        <li><a href="{{ route('customer.studio.index') }}" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] transition-colors">Studio</a></li>
                        <li><a href="{{ route('customer.layanan.index') }}" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] transition-colors">Layanan</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2.5">
                        <li><a href="{{ route('about') }}" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] transition-colors">Tentang Kami</a></li>
                        <li><a href="{{ route('pricing') }}" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] transition-colors">Harga</a></li>
                        <li><a href="{{ route('contact') }}" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] transition-colors">Kontak</a></li>
                    </ul>
                </div>
            ...
            <div class="border-t border-[#e5e5e7] mt-10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
                <p class="text-xs text-[#86868b]">&copy; {{ date('Y') }} Stekpro Multimedia & Broadcast. All rights reserved.</p>
    ...
    <a href="https://wa.me/{{ $waPhone }}" target="_blank" rel="noopener" class="whatsapp-btn fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#25D366] text-white flex items-center justify-center shadow-lg hover:bg-[#20BD5A] transition-colors">

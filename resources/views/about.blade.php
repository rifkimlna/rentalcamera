            ...
            @foreach($advantages as $i => $adv)
                <div class="card-apple-static p-6 lg:p-8">
                    <div class="w-10 h-10 rounded-2xl bg-[#f5f5f7] flex items-center justify-center mb-4">
                        <span class="text-lg font-bold text-[#1d1d1f]">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <h3 class="text-base font-semibold text-[#1d1d1f] mb-2">{{ $adv['title'] }}</h3>
                    <p class="text-sm text-[#6e6e73] leading-relaxed">{{ $adv['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </div>
        ...
        @if($portfolios->count() > 0)
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($portfolios as $item)
                    <div class="group relative overflow-hidden rounded-2xl bg-[#e5e5e7] aspect-[4/3] cursor-pointer" onclick="openPortfolioModal('{{ $item->id }}')">
                        @if($item->tipe === 'foto')
                            @if($item->embed_url)
                                <iframe src="{{ $item->embed_url }}" class="w-full h-full object-cover" loading="lazy" title="{{ $item->judul }}"></iframe>
                            @elseif($item->gambar_url)
                                <img src="{{ $item->gambar_url }}" alt="{{ $item->judul }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy">
                            @else
                        ...
                        @else
                            @if($item->embed_url)
                                <iframe src="{{ $item->embed_url }}" class="w-full h-full" loading="lazy" allowfullscreen title="{{ $item->judul }}"></iframe>
                            @elseif($item->gambar_url)
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <div class="absolute bottom-0 left-0 right-0 p-3 text-white">
                                ...
                                @if($item->deskripsi)
                                    <p class="text-xs text-white/60 truncate">{{ $item->deskripsi }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            @foreach($team as $member)
                <div class="text-center">
                    <div class="w-20 h-20 rounded-full bg-[#f5f5f7] mx-auto mb-4 flex items-center justify-center">
                        <span class="text-2xl font-bold text-[#d1d1d6]">{{ substr($member['name'], 0, 1) }}</span>
                    </div>
                    <p class="text-sm font-semibold text-[#1d1d1f]">{{ $member['name'] }}</p>
                    <p class="text-xs text-[#86868b] mt-0.5">{{ $member['role'] }}</p>
                </div>
            @endforeach
        </div>
    </div>

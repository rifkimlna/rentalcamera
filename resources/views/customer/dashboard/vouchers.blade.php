                ...
                <div class="p-4">
                    <div class="flex items-start justify-between gap-2 mb-2">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-[#1d1d1f] mt-0.5">{{ $voucher->kode_voucher }}</p>
                        </div>
                        <span class="badge-apple !text-[10px] !px-2 !py-0.5 border border-[#e5e5e7]">{{ $voucher->type_label }}</span>
                    </div>
                    <div class="text-xs text-[#6e6e73] space-y-1 mb-3">
                        <p>Diskon: <span class="text-[#86868b] font-medium">{{ $voucher->value_formatted }}</span></p>
                        @if($voucher->min_purchase > 0)
                        <p>Min. Belanja: <span class="text-[#86868b] font-medium">{{ $voucher->min_purchase_formatted }}</span></p>
                        @endif
                        @if($voucher->max_discount)
                        <p>Maks. Diskon: <span class="text-[#86868b] font-medium">{{ $voucher->max_discount_formatted }}</span></p>
                        @endif
                        <p>Berlaku: <span class="text-[#86868b] font-medium">{{ $voucher->start_date->format('d M Y') }} - {{ $voucher->end_date->format('d M Y') }}</span></p>
                    </div>
                    <button onclick="copyVoucher('{{ $voucher->kode_voucher }}')" class="btn-outline-apple w-full">
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-[#86868b]">{{ $usage->voucher->nama_voucher ?? '' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-medium">-{{ $usage->discount_amount_formatted }}</p>
                            <p class="text-xs text-[#86868b]">{{ $usage->created_at->format('d M Y') }}</p>
                        </div>
                    </div>
                </div>
            </div>

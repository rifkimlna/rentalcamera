...
<div class="p-4">
    <div class="text-sm mb-4">
        <ul class="flex items-center gap-2 text-[#6e6e73]">
            <li><a href="{{ route('customer.dashboard') }}" class="hover:text-[#1d1d1f] transition-all">Dashboard</a></li>
            <li>/</li>
            @if($isStudioBooking)
    ...
    @if(!$isStudioBooking && !$isLayananBooking && count($keranjangItems) == 0)
                    ...
                    <div class="card-apple-static">
                        <div class="p-4 sm:p-6">
                            <h5 class="font-semibold text-sm flex items-center gap-2 mb-4">
                                ...
                                Jadwal Booking {{ $isStudioBooking ? 'Studio' : 'Layanan' }}
                            </h5>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 input-apple w-full">
                                        ...
                                        <span class="text-sm">{{ \Carbon\Carbon::parse($bookingData['tanggal_booking'])->format('d M Y') }}</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 input-apple w-full">
                                        ...
                                        <span class="text-sm">{{ $bookingData['jam_mulai'] }}</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 input-apple w-full">
                                        ...
                                        <span class="text-sm">{{ $bookingData['jam_selesai'] }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 input-apple w-full">
                                        ...
                                        <span class="text-sm">{{ \Carbon\Carbon::parse($firstItem->tanggal_sewa)->format('d M Y') }}</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 input-apple w-full">
                                        ...
                                        <span class="text-sm">{{ $firstItem->jam_mulai ?? '08:00' }}</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 input-apple w-full">
                                        ...
                                        <span class="text-sm">{{ \Carbon\Carbon::parse($firstItem->tanggal_kembali)->format('d M Y') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" class="input-apple w-full" name="nama_customer" value="{{ auth()->user()->nama }}" required>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-[#6e6e73] mb-1">Email</label>
                                    <input type="email" class="input-apple w-full" name="email_customer" value="{{ auth()->user()->email }}" required>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-[#6e6e73] mb-1">No. Telepon</label>
                                    <input type="tel" class="input-apple w-full" name="telepon_customer" value="{{ auth()->user()->telepon }}" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-apple-static">
                        <div class="p-4 sm:p-6">
                            <h5 class="font-semibold text-sm flex items-center gap-2 mb-4">
                                ...
                                Detail {{ $isStudioBooking ? 'Studio' : 'Layanan' }}
                            </h5>
                            <div class="flex items-center justify-between py-2">
                                <div class="flex-1 min-w-0">
                                    <h6 class="text-sm font-medium">{{ $bookingData['nama_studio'] ?? $bookingData['nama_layanan'] }}</h6>
                                    @if($bookingData['tipe_booking'] === 'paket' && $bookingData['paket'])
                                        <p class="text-xs text-[#6e6e73]">
                                            Paket: {{ $bookingData['paket']['nama_paket'] }} ({{ $bookingData['paket']['durasi_jam'] }} jam)
                                        </p>
                                    @else
                                        <p class="text-xs text-[#6e6e73]">
                                            {{ $isStudioBooking ? 'Sewa per Jam' : 'Sewa Layanan' }} - {{ $bookingData['durasi_jam'] }} jam x {{ number_format($bookingData['harga_per_jam'] ?? $bookingData['harga_mulai'], 0, ',', '.') }}
                                        </p>
                                    @endif
                                </div>
                                <div class="text-right shrink-0 ml-4">
                                    <span class="text-sm font-bold">Rp {{ number_format($bookingData['total_harga'], 0, ',', '.') }}</span>
                                </div>
                            </div>
                            <div class="mt-3">
                            ...
                            <div class="divide-y divide-[#f0f0f2]">
                                @foreach($keranjangItems as $item)
                                    ...
                                    <div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                                        <div class="flex-1 min-w-0">
                                            <h6 class="text-sm font-medium truncate">{{ optional($item->produk)->nama_produk ?? 'Produk tidak tersedia' }}</h6>
                                            <p class="text-xs text-[#6e6e73]">
                                                Rp {{ number_format($harga, 0, ',', '.') }} x {{ $item->jumlah }} barang x {{ $days }} hari
                                            </p>
                                        </div>
                                        <div class="text-right shrink-0 ml-4">
                                            <span class="text-sm font-bold">Rp {{ number_format($subtotalItem, 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="p-4 sm:p-6">
                            ...
                            <div class="space-y-2 text-sm mb-4">
                                <div class="flex justify-between">
                                    <span class="text-[#6e6e73]">Subtotal {{ $isStudioBooking ? '(' . $totalJam . ' jam)' : '(' . $totalDays . ' hari)' }}</span>
                                    <span class="font-medium">Rp {{ number_format($totalSubtotal, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-base" id="totalAmount">Rp {{ number_format($totalSubtotal, 0, ',', '.') }}</span>
                                ...
                                <div class="space-y-2">
                                    @foreach($paymentMethods as $method)
                                        <label class="flex items-start gap-3 cursor-pointer p-3 rounded-xl border border-[#f0f0f2] has-checked:border-[#1d1d1f] has-checked:bg-[#f5f5f7] transition-all">
                                            <input class="radio radio-sm mt-0.5 payment-method" type="radio" name="payment_method_id" id="method{{ $method->id }}" value="{{ $method->id }}"
                                                   data-fee-percentage="{{ $method->fee_percentage }}" data-fee-flat="{{ $method->fee_flat }}" {{ $loop->first ? 'checked' : '' }}>
                                            <div class="flex-1">
                                                <strong class="text-sm">{{ $method->name }}</strong>
                                                @if($method->fee_percentage > 0 || $method->fee_flat > 0)
                                                    <small class="text-[#6e6e73] block text-xs">
                                                        Admin: @if($method->fee_percentage > 0){{ $method->fee_percentage }}%@endif @if($method->fee_flat > 0)+ Rp {{ number_format($method->fee_flat, 0, ',', '.') }}@endif
                                                    </small>
                                                @endif
                                            </div>
                                        </label>

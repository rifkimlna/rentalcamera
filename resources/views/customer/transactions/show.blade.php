...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between gap-3">
        <div class="min-w-0">
            <h1 class="text-lg sm:text-xl font-bold text-[#1d1d1f] tracking-tight">Detail {{ $isSewa ? 'Transaksi' : 'Booking' }}</h1>
            <p class="text-xs font-mono text-[#86868b] mt-0.5">{{ $kode }}</p>
        </div>
        <a href="{{ route('customer.transactions.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium px-4 py-2 rounded-full bg-[#f5f5f7] hover:bg-[#e5e5e7] text-[#1d1d1f] transition-colors shrink-0">
    ...
    {{-- Banner Status --}}
    <div class="{{ $sc['bg'] }} border {{ $sc['border'] }} rounded-2xl p-4 sm:p-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3.5">
                <div class="w-11 h-11 shrink-0 rounded-full bg-white/70 flex items-center justify-center {{ $sc['text'] }}">
                ...
                <div class="min-w-0">
                    <p class="font-semibold {{ $sc['text'] }}">{{ $statusTransLabel }}</p>
                    <p class="text-xs {{ $sc['text'] }} opacity-80 mt-0.5">
                        {{ $statusBayarLabel }}
                        @if($paidAt)
                            &middot; {{ \Carbon\Carbon::parse($paidAt)->translatedFormat('d M Y H:i') }}
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2 shrink-0">
                @if($isSewa && $statusBayar == 'pending' && $statusTrans == 'menunggu_pembayaran')
                    <a href="{{ route('customer.checkout.payment', $t->id) }}" class="btn-dark-apple !py-2 !px-5 !text-sm">
                ...
                @elseif(!$isSewa && $statusBayar === 'pending' && $statusTrans === 'pending')
                    <a href="{{ route($isStudio ? 'customer.studio.payment' : 'customer.layanan.payment', $t->id) }}" class="btn-dark-apple !py-2 !px-5 !text-sm">
                        <svg class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                        Bayar Sekarang
                    </a>
                @endif

                @if($isSewa && $t->canBeCancelled())
                    <form method="POST" action="{{ route('customer.transactions.cancel', $t->id) }}" onsubmit="return confirm('Yakin ingin membatalkan transaksi ini?')">
                        @csrf
                ...
                @elseif(!$isSewa && $statusBayar === 'pending' && $statusTrans === 'pending')
                    <form method="POST" action="{{ route($isStudio ? 'customer.studio.booking.cancel' : 'customer.layanan.booking.cancel', $t->id) }}" onsubmit="return confirm('Batalkan booking ini?')">
                        @csrf @method('PUT')
                        <button type="submit" class="inline-flex items-center justify-center text-sm font-medium px-5 py-2 rounded-full bg-[#d70015] text-white hover:bg-[#bf0013] transition-colors">Batalkan</button>
                    </form>
                @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-6">
        ...
        <div class="lg:col-span-2 space-y-5 lg:space-y-6">
            ...
            <section class="card-apple-static p-4 sm:p-6">
                ...
                <ul class="divide-y divide-[#f0f0f2]">
                    @foreach($t->detailTransaksis as $detail)
                        @php $review = $t->reviews->firstWhere('produk_id', $detail->produk_id); @endphp
                        <li class="py-4 first:pt-0 last:pb-0">
                            <div class="flex items-start gap-3.5">
                                <div class="shrink-0 w-14 h-14 rounded-xl overflow-hidden bg-[#f5f5f7]">
                                    @if($detail->produk && $detail->produk->gambar_utama)
                                        <img src="{{ asset('storage/' . $detail->produk->gambar_utama) }}" alt="{{ $detail->nama_produk }}" class="w-full h-full object-cover">
                                    @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-[#1d1d1f]">{{ $detail->nama_produk }}</p>
                                    <p class="text-xs text-[#86868b] mt-0.5">
                                        Rp {{ number_format($detail->harga_per_hari, 0, ',', '.') }}/hari
                                        &times; {{ $detail->jumlah }} barang
                                        &times; {{ $detail->lama_sewa }} hari
                                    </p>
                                </div>
                                ...
                                <div class="shrink-0 text-right">
                                    ...
                                    @elseif(in_array($statusTrans, ['selesai', 'completed']))
                                        <a href="{{ route('customer.reviews.create', ['transaction' => $t->id]) }}?product={{ $detail->produk_id }}"
                                    ...
                                    @else
                                        <span class="text-sm font-bold text-[#1d1d1f]">Rp {{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            @if($review)
                                <div class="mt-3 ml-[62px] p-3.5 rounded-xl bg-[#f5f5f7]">
                                    <div class="flex items-center gap-1 mb-1.5" aria-label="Rating {{ $review->rating }} dari 5">
                                        @for($i = 1; $i <= 5; $i++)
                                            <svg class="h-3.5 w-3.5 {{ $i <= round($review->rating) ? 'text-[#ff9500]' : 'text-[#d1d1d6]' }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                        @endfor
                                        <span class="text-[11px] text-[#86868b] ml-1">{{ $review->rating }}/5</span>
                                    </div>
                                    @if($review->judul)
                                        <p class="text-sm font-medium text-[#1d1d1f]">{{ $review->judul }}</p>
                                    @endif
                                    <p class="text-xs text-[#6e6e73] mt-0.5 leading-relaxed">{{ $review->komentar }}</p>
                                </div>
                            @endif
                        </li>
                    @endforeach
            ...
            @else
            {{-- Detail Booking Studio / Layanan --}}
            <section class="card-apple-static p-4 sm:p-6">
                <h3 class="text-xs font-semibold text-[#86868b] uppercase tracking-wider mb-4">Detail {{ $isStudio ? 'Studio' : 'Layanan' }}</h3>
                <div class="flex items-start gap-4">
                    <div class="shrink-0 w-20 h-20 rounded-2xl overflow-hidden bg-[#f5f5f7]">
                        @php $gambar = $isStudio ? $t->studio?->gambar_utama : $t->layanan?->gambar_utama; @endphp
                        @if($gambar)
                            <img src="{{ asset('storage/' . $gambar) }}" class="w-full h-full object-cover" alt="{{ $namaItem }}">
                        @else
                    ...
                    <div class="flex-1 min-w-0">
                        <h4 class="text-base font-semibold text-[#1d1d1f]">{{ $namaItem }}</h4>
                        <p class="text-sm text-[#6e6e73] mt-1">
                            {{ \Carbon\Carbon::parse($t->tanggal_booking)->translatedFormat('d M Y') }}
                            &middot; {{ $t->jam_mulai ? \Carbon\Carbon::parse($t->jam_mulai)->format('H:i') : '-' }}
                            &ndash; {{ $t->jam_selesai ? \Carbon\Carbon::parse($t->jam_selesai)->format('H:i') : '-' }}
                            &middot; {{ $t->durasi_jam }} jam
                        </p>
                        <span class="inline-flex items-center text-[11px] font-medium px-2.5 py-1 rounded-full bg-[#f5f5f7] text-[#6e6e73] mt-2.5">
                            {{ $t->tipe_booking_label }}
                            @if($isStudio && $t->paketStudio)
                                &middot; {{ $t->paketStudio->nama_paket }}
                            @elseif($isLayanan && $t->paketLayanan)
                                &middot; {{ $t->paketLayanan->nama_paket }}
                            @endif
                        </span>
                    </div>
                </div>
                    ...
                    @foreach($steps as $key => $label)
                        ...
                        <li class="flex items-center gap-2.5 lg:flex-1 lg:last:flex-none">
                            <span class="w-6 h-6 shrink-0 rounded-full flex items-center justify-center text-[10px] font-bold
                                {{ $isCompleted ? 'bg-[#1d1d1f] text-white' : 'bg-[#e5e5e7] text-[#86868b]' }}">
                            ...
                            <span class="text-xs whitespace-nowrap {{ $isCurrent ? 'font-semibold text-[#1d1d1f]' : ($isCompleted ? 'font-medium text-[#1d1d1f]' : 'text-[#86868b]') }}">{{ $label }}</span>
                            @unless($isLast)
                                <span class="hidden lg:block flex-1 h-px mx-3 {{ $isCompleted ? 'bg-[#1d1d1f]' : 'bg-[#e5e5e7]' }}" aria-hidden="true"></span>
                            @endunless
                        </li>
                    @endforeach
                </ol>

                @if($isSewa && $t->canReturn())
                    <div class="mt-5 p-4 bg-[#f0faf1] border border-[#d1f5d5] rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <form method="POST" action="{{ route('customer.transactions.confirm-return', $t->id) }}" class="shrink-0" onsubmit="return confirm('Konfirmasi bahwa barang sudah dikembalikan?')">
                            @csrf
        ...
        <div class="space-y-5 lg:space-y-6">
            ...
            <section class="card-apple-static p-4 sm:p-6">
                ...
                <dl class="space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <dd><span class="inline-flex text-[11px] font-medium px-2.5 py-1 rounded-full {{ $transBadge }}">{{ $statusTransLabel }}</span></dd>
                    </div>
                    <div class="flex items-center justify-between">
                        <dt class="text-[#86868b]">Pembayaran</dt>
                        <dd><span class="inline-flex text-[11px] font-medium px-2.5 py-1 rounded-full {{ $bayarBadge }}">{{ $statusBayarLabel }}</span></dd>
                    </div>
                    @if($isSewa)
                        <div class="flex items-center justify-between pt-1 border-t border-[#f0f0f2]">
                            <dt class="text-[#86868b]">Tanggal Sewa</dt>
                            <dd class="font-medium text-[#1d1d1f]">{{ \Carbon\Carbon::parse($t->tanggal_pengambilan)->translatedFormat('d M Y') }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                            <dt class="text-[#86868b]">Kembali</dt>
                            <dd class="font-medium text-[#1d1d1f]">{{ \Carbon\Carbon::parse($t->tanggal_pengembalian)->translatedFormat('d M Y') }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                            <dt class="text-[#86868b]">Lama Sewa</dt>
                            <dd class="font-medium text-[#1d1d1f]">{{ $t->lama_sewa }} hari</dd>
                        </div>
                        <div class="flex items-center justify-between">
                    ...
                    @else
                        <div class="flex items-center justify-between pt-1 border-t border-[#f0f0f2]">
                            ...
                            <dd class="font-medium text-[#1d1d1f]">{{ \Carbon\Carbon::parse($t->tanggal_booking)->translatedFormat('d M Y') }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                            <dt class="text-[#86868b]">Jam</dt>
                            <dd class="font-medium text-[#1d1d1f]">{{ $t->jam_mulai ? \Carbon\Carbon::parse($t->jam_mulai)->format('H:i') : '-' }} &ndash; {{ $t->jam_selesai ? \Carbon\Carbon::parse($t->jam_selesai)->format('H:i') : '-' }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                            <dt class="text-[#86868b]">Durasi</dt>
                            <dd class="font-medium text-[#1d1d1f]">{{ $t->durasi_jam }} jam</dd>
                        </div>
                        <div class="flex items-center justify-between">
                            <dt class="text-[#86868b]">Tipe</dt>
                            <dd class="font-medium text-[#1d1d1f]">{{ $t->tipe_booking_label }}</dd>
                        </div>
                    @endif
                </dl>
            </section>
            ...
            @if($t->paymentMethod)
                <section class="card-apple-static p-4 sm:p-6">
                    ...
                    <div class="flex items-center gap-3.5 p-4 bg-[#f5f5f7] rounded-xl">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-[#1d1d1f] truncate">{{ $t->paymentMethod->name }}</p>
                            @if($isSewa && $t->va_number)
                                <p class="text-xs text-[#6e6e73] font-mono mt-0.5">VA: {{ $t->va_number }}</p>
                            @endif
                            @if($isSewa && $t->bank)
                                <p class="text-xs text-[#6e6e73] mt-0.5">Bank: {{ $t->bank }}</p>
                            @endif
                        </div>
                    </div>
                </section>
                ...
                <div class="space-y-2.5 text-sm">
                    @if($isSewa)
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-[#1d1d1f]">Rp {{ number_format($t->subtotal, 0, ',', '.') }}</span>
                        </div>
                        @if($t->diskon > 0)
                            <div class="flex justify-between">
                                <span class="text-[#86868b]">Diskon</span>
                                <span class="font-medium text-[#248a3d]">&minus;Rp {{ number_format($t->diskon, 0, ',', '.') }}</span>
                            </div>
                        @endif
                        @if($t->biaya_lainnya > 0)
                            <div class="flex justify-between">
                                <span class="text-[#86868b]">Biaya Lainnya</span>
                                <span class="font-medium text-[#1d1d1f]">Rp {{ number_format($t->biaya_lainnya, 0, ',', '.') }}</span>
                            </div>
                        @endif
                    @else
                        <div class="flex justify-between">
                            <span class="text-[#86868b]">Biaya Sewa</span>
                            <span class="font-medium text-[#1d1d1f]">{{ $t->total_harga_formatted }}</span>
                        </div>
                        @if($t->diskon_voucher > 0)
                            <div class="flex justify-between">
                                <span class="text-[#86868b]">Diskon Voucher</span>
                                <span class="font-medium text-[#248a3d]">&minus;{{ $t->diskon_voucher_formatted }}</span>
                            </div>
                        @endif
                    @endif
                    @if($t->admin_fee > 0)
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-[#1d1d1f]">Rp {{ number_format($t->admin_fee, 0, ',', '.') }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between pt-3 border-t border-[#e5e5e7]">
                        <span class="font-semibold text-[#1d1d1f]">Total</span>
                        <span class="font-bold text-base text-[#1d1d1f] tracking-tight">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
                    </div>
                </div>
            </section>
            ...
            @if($catatan)
                <section class="card-apple-static p-4 sm:p-6">
                    ...
                    <p class="text-sm text-[#494950] leading-relaxed whitespace-pre-line">{{ $catatan }}</p>
                </section>
            @endif

            {{-- Aksi --}}
            <div class="flex flex-col gap-2.5">
                <a href="https://wa.me/{{ $waPhone }}?text={{ rawurlencode("Halo, saya mau tanya tentang {$kode}\n{$namaItem}\nStatus: {$statusTransLabel}\nTotal: Rp " . number_format($grandTotal, 0, ',', '.')) }}"
                ...
                @if($isSewa && in_array($statusBayar, ['settlement', 'capture']))
                    <a href="{{ route('customer.transactions.invoice', $t->id) }}" target="_blank" class="btn-outline-apple w-full justify-center">

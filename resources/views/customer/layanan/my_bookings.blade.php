...
@section('title', 'Booking Layanan Saya')
@section('page-title', 'Booking Layanan Saya')
...
<div class="space-y-4">
    <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <a href="{{ route('customer.layanan.index') }}" class="btn-dark-apple btn-sm">
                ...
                <div class="p-4">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-[#f5f5f7] hidden sm:block">
                            @if($imgUrl)
                                <img src="{{ $imgUrl }}" class="w-full h-full object-cover" alt="{{ $booking->layanan->nama_layanan }}">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <h4 class="text-sm font-medium">{{ $booking->layanan->nama_layanan }}</h4>
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-2">
                                <span class="text-xs text-[#6e6e73]">
                                    ...
                                    @if($booking->paketLayanan)
                                        &middot; {{ $booking->paketLayanan->nama_paket }}
                                    @endif
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-3">
                                @if($isPending)
                                    <a href="{{ route('customer.layanan.payment', $booking->id) }}" class="btn-dark-apple btn-xs">
                                    ...
                                    <form method="POST" action="{{ route('customer.layanan.booking.cancel', $booking->id) }}" class="inline" onsubmit="return confirm('Batalkan booking ini?')">
                                        @csrf @method('PUT')
        ...
        @empty
            <div class="card bg-white border border-[#e5e5e7]">
                <div class="p-5 text-center py-12">
                    ...
                    <a href="{{ route('customer.layanan.index') }}" class="btn-dark-apple btn-sm">Lihat Layanan</a>
                </div>
            </div>
        @endforelse
    </div>

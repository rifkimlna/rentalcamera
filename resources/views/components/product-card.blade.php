...
<div class="card-apple overflow-hidden flex flex-col">
    <a href="{{ $detailUrl }}" class="group relative overflow-hidden bg-[#f5f5f7] aspect-square block" aria-label="{{ $product->nama_produk }}">
        @if($product->gambar_utama)
            <img src="{{ asset('storage/' . $product->gambar_utama) }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="{{ $product->nama_produk }}" loading="lazy">
        @else
        ...
        <div class="absolute bottom-0 left-0 right-0 p-2.5 sm:p-3 lg:p-4 text-white">
            ...
            <div class="flex items-center gap-0.5 sm:gap-1 mt-0.5 sm:mt-1">
                @for($i = 1; $i <= 5; $i++)
                    <svg class="h-2.5 w-2.5 sm:h-3 sm:w-3 {{ $i <= round($product->rating) ? 'text-[#ff9500]' : 'text-white/30' }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                @endfor
    ...
    <div class="p-2.5 sm:p-3 lg:p-4 bg-white flex flex-col flex-1">
        ...
        <div class="flex gap-1.5 sm:gap-2 mt-auto">
            <a href="{{ $detailUrl }}" class="flex-1 text-center text-[11px] sm:text-xs lg:text-sm font-medium py-1.5 sm:py-2 rounded-lg lg:rounded-xl border border-[#e5e5e7] text-[#1d1d1f] hover:bg-[#f5f5f7] transition-colors">Detail</a>
            @if($product->status == 'available' && $product->stok_tersedia > 0)
                <a href="{{ $detailUrl }}" class="flex-1 text-center text-[11px] sm:text-xs lg:text-sm font-medium py-1.5 sm:py-2 rounded-lg lg:rounded-xl bg-[#1d1d1f] text-white hover:bg-[#333] transition-colors">Sewa</a>
            @else

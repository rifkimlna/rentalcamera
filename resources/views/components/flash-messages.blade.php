...
@if(session('success') || session('error') || session('warning') || session('info') || ($showErrors && $errors->any()))
    <div class="space-y-3 mb-5" aria-live="polite">
        @foreach($flashTypes as $key => $type)
            @if(session($key))
                <div role="alert" {{ $type['persistent'] ? 'data-persistent="true"' : '' }} class="flex items-start gap-3 px-4 py-3 rounded-xl border {{ $type['bg'] }} text-sm">
                    <svg class="w-4 h-4 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">{!! $type['icon'] !!}</svg>

<?php $__env->startSection('title', 'Pembayaran - Stekpro Multimedia & Broadcast'); ?>
    ...
    <div class="mx-auto w-full max-w-2xl">
            ...
            <div class="card-apple-static mb-4">
                <div class="p-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <div class="mb-3">
                                ...
                                <h5 class="font-bold text-[#0071e3]"><?php echo e($transaksi->kode_transaksi); ?></h5>
                            </div>
                            <div class="mb-3">
                                <small class="text-[#6e6e73] block">Tanggal Transaksi</small>
                                <h6><?php echo e(\Carbon\Carbon::parse($transaksi->created_at)->translatedFormat('d F Y H:i')); ?></h6>
                            </div>
                        </div>
                        <div>
                            <div class="mb-3">
                                <small class="text-[#6e6e73] block">Total Pembayaran</small>
                                <h3 class="font-bold text-[#6e6e73]">Rp <?php echo e(number_format($transaksi->grand_total, 0, ',', '.')); ?></h3>
                            </div>
                            <div class="mb-3">
                                <small class="text-[#6e6e73] block">Status</small>
                                <span class="badge-apple <?php echo e($transaksi->status_pembayaran == 'pending' ? 'badge-warning' : 'badge-success'); ?>">
                    ...
                    <!-- Payment Expiry -->
                    <?php if($transaksi->payment_expired_at): ?>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="mb-0 text-sm">
                                        ...
                                        <strong><?php echo e(\Carbon\Carbon::parse($transaksi->payment_expired_at)->translatedFormat('d F Y H:i')); ?></strong>
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div class="card-apple-static mb-4">
                <div class="p-5">
                    ...
                    <?php if($transaksi->payment_type == 'bank_transfer' && $transaksi->va_number): ?>
                            ...
                            <div class="bg-white p-4 rounded-xl mb-3">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                                    <div>
                                        ...
                                        <h5 class="font-bold mb-0"><?php echo e(strtoupper($transaksi->bank)); ?></h5>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <div class="min-w-0 flex-1">
                                            <small class="text-[#6e6e73] block">Nomor Virtual Account</small>
                                            <h5 class="font-bold mb-0 break-all"><?php echo e($transaksi->va_number); ?></h5>
                                        </div>
                                        <button type="button" id="copy-va" data-va="<?php echo e($transaksi->va_number); ?>" class="btn-outline-apple shrink-0 !text-xs" title="Salin Nomor VA">
        ...
        data-success-url="<?php echo e(route('customer.checkout.success', $transaksi->id)); ?>"
        data-check-url="<?php echo e(route('customer.checkout.check-status', $transaksi->id)); ?>"
        data-transactions-url="<?php echo e(route('customer.transactions.index')); ?>">
                        ...
                        <?php else: ?>
                            ...
                            <div class="mb-4">
                                <a href="<?php echo e(route('customer.checkout.payment', $transaksi->id)); ?>" class="btn-dark-apple w-full md:w-auto md:px-8">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 me-2 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <table class="w-full">
                                ...
                                <?php $__currentLoopData = $transaksi->detailTransaksis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="border-t border-[#f0f0f2]">
                                        <td class="py-3 px-2">
                                            <div class="flex items-center gap-3">
                                                <div class="shrink-0">
                                                    <?php if($detail->produk->gambar_utama): ?>
                                                ...
                                                <div>
                                                    <h6 class="mb-1 font-semibold"><?php echo e($detail->nama_produk); ?></h6>
                                                    <small class="text-[#6e6e73]">
                                                        <?php echo e($detail->lama_sewa); ?> hari x Rp <?php echo e(number_format($detail->harga_per_hari, 0, ',', '.')); ?>/hari
                                                    </small>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="py-3 px-2">
                                            <?php echo e(\Carbon\Carbon::parse($transaksi->tanggal_pengambilan)->translatedFormat('d M Y')); ?><br>
                                            <small class="text-[#6e6e73]">s/d</small><br>
                                            <?php echo e(\Carbon\Carbon::parse($transaksi->tanggal_pengembalian)->translatedFormat('d M Y')); ?>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="text-right py-2 px-2">Rp <?php echo e(number_format($transaksi->subtotal, 0, ',', '.')); ?></td>
                                </tr>
                                <?php if($transaksi->diskon > 0): ?>
                                    <tr>
                                        <td colspan="2" class="text-right py-2 px-2"><strong>Diskon:</strong></td>
                                        <td class="text-right py-2 px-2 text-[#6e6e73]">-Rp <?php echo e(number_format($transaksi->diskon, 0, ',', '.')); ?></td>
                                    </tr>
                                <?php endif; ?>
                                <?php if($transaksi->biaya_asuransi > 0): ?>
                                    <tr>
                                        <td colspan="2" class="text-right py-2 px-2"><strong>Biaya Asuransi:</strong></td>
                                        <td class="text-right py-2 px-2">Rp <?php echo e(number_format($transaksi->biaya_asuransi, 0, ',', '.')); ?></td>
                                    </tr>
                                <?php endif; ?>
                                <tr class="bg-[#f5f5f7]">
                                    <td colspan="2" class="text-right py-2 px-2"><strong>Total:</strong></td>
                                    <td class="text-right py-2 px-2 font-bold text-[#6e6e73]">
                                        Rp <?php echo e(number_format($transaksi->grand_total, 0, ',', '.')); ?>
                    ...
                    <div class="md:hidden space-y-3">
                            ...
                            <div class="bg-[#f5f5f7] rounded-xl p-3">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h6 class="font-semibold text-sm leading-snug"><?php echo e($detail->nama_produk); ?></h6>
                                        <p class="text-xs text-[#6e6e73] mt-0.5"><?php echo e($detail->lama_sewa); ?> hari x Rp <?php echo e(number_format($detail->harga_per_hari, 0, ',', '.')); ?>/hari</p>
                                        <p class="text-xs text-[#6e6e73] mt-0.5">
                                            <?php echo e(\Carbon\Carbon::parse($transaksi->tanggal_pengambilan)->translatedFormat('d M Y')); ?>

                                            <span class="mx-1">-</span>
                                            <?php echo e(\Carbon\Carbon::parse($transaksi->tanggal_pengembalian)->translatedFormat('d M Y')); ?>

                                        </p>
                                        <p class="font-bold text-sm mt-1">Rp <?php echo e(number_format($detail->subtotal, 0, ',', '.')); ?></p>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="bg-[#f5f5f7] rounded-xl p-3 space-y-1.5">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-semibold">Rp <?php echo e(number_format($transaksi->subtotal, 0, ',', '.')); ?></span>
                            </div>
                            <?php if($transaksi->diskon > 0): ?>
                                <div class="flex justify-between text-sm">
                                    <span class="text-[#6e6e73]">Diskon</span>
                                    <span class="font-semibold text-[#6e6e73]">-Rp <?php echo e(number_format($transaksi->diskon, 0, ',', '.')); ?></span>
                                </div>
                            <?php endif; ?>
                            <?php if($transaksi->biaya_asuransi > 0): ?>
                                <div class="flex justify-between text-sm">
                                    <span class="text-[#6e6e73]">Biaya Asuransi</span>
                                    <span class="font-semibold">Rp <?php echo e(number_format($transaksi->biaya_asuransi, 0, ',', '.')); ?></span>
                                </div>
                            <?php endif; ?>
                            <div class="flex justify-between text-sm font-bold border-t border-[#e5e5e7] pt-1.5 mt-1.5">
                                <span>Total</span>
                                <span class="text-[#6e6e73]">Rp <?php echo e(number_format($transaksi->grand_total, 0, ',', '.')); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-4 flex justify-center">
                <a href="<?php echo e(route('customer.transactions.show', $transaksi->id)); ?>" 
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\PROJECT KP\resources\views\customer\checkout\payment.blade.php ENDPATH**/ ?>
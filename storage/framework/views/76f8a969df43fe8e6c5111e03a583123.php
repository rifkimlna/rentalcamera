...
<?php $__env->startSection('title', 'Layanan - Stekpro Multimedia & Broadcast'); ?>
...
<div>
    ...
    <div class="card-apple-static mb-4 sm:mb-6 p-3 sm:p-4 lg:p-5">
        ...
        <form method="GET" class="hidden lg:grid lg:grid-cols-3 gap-4 items-end">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="<?php echo e(route('customer.layanan.index')); ?>" class="btn-outline-apple">
        ...
        <form method="GET" class="lg:hidden flex items-center gap-2">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" class="input-apple !pl-9" placeholder="Cari layanan..." value="<?php echo e(request('search')); ?>">
            </div>
            <a href="<?php echo e(route('customer.layanan.index')); ?>" class="btn-outline-apple !px-3" title="Reset">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            </a>
    ...
    <?php $__empty_1 = true; $__currentLoopData = $layanans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $layanan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="card-apple-static rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <!-- Image -->
            <a href="<?php echo e(route('customer.layanan.show', $layanan->slug)); ?>" class="group relative overflow-hidden bg-[#f5f5f7] aspect-square block">
                <?php if($layanan->gambar_utama): ?>
                    <img src="<?php echo e(asset('storage/' . $layanan->gambar_utama)); ?>"
                         class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="<?php echo e($layanan->nama_layanan); ?>">
                <?php else: ?>
                ...
                <div class="absolute bottom-0 left-0 right-0 p-2.5 sm:p-3 lg:p-4 text-white">
                    <p class="text-xs sm:text-sm lg:text-base font-semibold truncate"><?php echo e($layanan->nama_layanan); ?></p>
                    <div class="flex items-center gap-0.5 sm:gap-1 mt-0.5 sm:mt-1">
                        <div class="flex items-center gap-0.5">
                            <?php for($i = 1; $i <= 5; $i++): ?>
                                <svg class="h-2.5 w-2.5 sm:h-3 sm:w-3 <?php echo e($i <= round($layanan->rating) ? 'text-[#ff9500]' : 'text-white/30'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                            <?php endfor; ?>
                        </div>
                        <span class="text-[10px] sm:text-xs text-white/80">(<?php echo e($layanan->rating); ?>)</span>
                    </div>
                </div>
            </a>
            ...
            <div class="p-2.5 sm:p-3 lg:p-4 bg-white">
                <div class="flex items-center justify-between gap-1.5 mb-1.5 sm:mb-2">
                    <div>
                        <span class="text-xs sm:text-sm lg:text-base font-bold text-[#1d1d1f]"><?php echo e($layanan->harga_mulai_formatted); ?></span>
                        <span class="text-[10px] sm:text-xs text-[#6e6e73]">/mulai</span>
                        <?php if($layanan->paket_active_count > 0): ?>
                            <span class="text-[10px] sm:text-xs text-[#86868b] block mt-0.5"><?php echo e($layanan->paket_active_count); ?> paket</span>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <?php if($layanan->kategori): ?>
                    <div class="flex flex-wrap gap-1 mb-2 sm:mb-3">
                        <span class="badge-apple !text-[9px] sm:!text-[10px] !px-1.5 sm:!px-2 !py-0"><?php echo e($layanan->kategori); ?></span>
                    </div>
                <?php endif; ?>

                <div class="flex gap-1.5 sm:gap-2">
                    <a href="<?php echo e(route('customer.layanan.show', $layanan->slug)); ?>" class="btn-outline-apple flex-1 !text-[11px] sm:!text-xs lg:!text-sm !py-1.5 sm:!py-2">Detail</a>
                    
                    <?php if(auth()->guard()->check()): ?>
                        <button type="button" class="btn-dark-apple flex-1 !text-[11px] sm:!text-xs lg:!text-sm !py-1.5 sm:!py-2" onclick="openLayananModal(<?php echo e($layanan->id); ?>)">Sewa</button>
                    <?php else: ?>
                        <a href="<?php echo e(route('customer.layanan.show', $layanan->slug)); ?>" class="btn-dark-apple flex-1 !text-[11px] sm:!text-xs lg:!text-sm !py-1.5 sm:!py-2 text-center">Sewa</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        <div class="col-span-full card-apple-static p-12 text-center">
            ...
            <a href="<?php echo e(route('customer.layanan.index')); ?>" class="btn-outline-apple !text-sm">Reset Pencarian</a>
        </div>
    <?php endif; ?>
    </div>
...
<script>
    const layanansData = <?php echo json_encode($layanansJson); ?>;
    const LAYANAN_IS_GUEST = <?php echo e(auth()->check() ? 'false' : 'true'); ?>;
    const LAYANAN_LOGIN_URL = '<?php echo e(route("login")); ?>';
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\PROJECT KP\resources\views/customer/layanan/index.blade.php ENDPATH**/ ?>
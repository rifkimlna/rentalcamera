...
<?php $__env->startSection('page-title', 'Transaksi Saya'); ?>
...
<div class="space-y-5">
    ...
    <div class="card-apple-static p-3 sm:p-4" x-data="{ filterOpen: false }">
        ...
        <form method="GET" action="<?php echo e(route('customer.transactions.index')); ?>" class="hidden sm:flex items-center gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari transaksi..." class="input-apple !pl-10">
            </div>
            <div class="w-48">
            ...
            <?php if(request()->anyFilled(['search', 'status'])): ?>
                <a href="<?php echo e(route('customer.transactions.index')); ?>" class="text-sm text-[#6e6e73] hover:text-[#1d1d1f] px-3 py-2 rounded-full hover:bg-[#f5f5f7] transition-colors shrink-0">Reset</a>
            <?php endif; ?>
        </form>

        
        <form method="GET" action="<?php echo e(route('customer.transactions.index')); ?>" class="sm:hidden">
            <div class="flex items-center gap-2">
                <div class="relative flex-1">
                    ...
                    <input type="text" id="search-mobile" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari transaksi..." class="input-apple !pl-9 !text-sm">
                </div>
            ...
            <div class="card-apple overflow-hidden hover:shadow-md hover:-translate-y-px transition-all duration-300">
                <div class="p-4 sm:p-5">
                    <div class="flex items-start gap-3.5 sm:gap-4">
                        ...
                        <a href="<?php echo e($transaction->detail_link); ?>" class="shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden bg-[#f5f5f7]" aria-hidden="true" tabindex="-1">
                            <?php if($itemImage): ?>
                                <img src="<?php echo e($itemImage); ?>" class="w-full h-full object-cover" alt="" loading="lazy">
                            <?php else: ?>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <a href="<?php echo e($transaction->detail_link); ?>" class="block">
                                        <h4 class="text-[15px] sm:text-base font-semibold text-[#1d1d1f] truncate hover:text-[#0071e3] transition-colors"><?php echo e($itemName); ?></h4>
                                    </a>
                                    <p class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-[#86868b]">
                                        <span class="font-medium text-[#6e6e73]"><?php echo e($transaction->tipe_label); ?></span>
                                        <span aria-hidden="true">&middot;</span>
                                        <span class="font-mono"><?php echo e($transaction->kode); ?></span>
                                        <span aria-hidden="true">&middot;</span>
                                        <span><?php echo e($createdAt); ?></span>
                                    </p>
                                </div>
                                <div class="text-right shrink-0">
                                    <p class="text-[15px] sm:text-lg font-bold text-[#1d1d1f] tracking-tight">Rp <?php echo e(number_format($transaction->total ?? 0, 0, ',', '.')); ?></p>
                                </div>
                            </div>

                            
                            <div class="flex flex-wrap items-center gap-2 mt-2.5">
                                <span class="inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full <?php echo e($badgeBg); ?> <?php echo e($badgeText); ?>">
                                    <span class="w-1.5 h-1.5 rounded-full bg-current opacity-70"></span>
                                    <?php echo e($statusLabel); ?>

                                </span>
                                <?php if(in_array($transaction->status_bayar, ['settlement', 'capture', 'paid']) && !in_array($statusKey, ['cancelled', 'dibatalkan'])): ?>
                                ...
                                <?php elseif($transaction->status_bayar == 'pending' && !in_array($statusKey, ['cancelled', 'dibatalkan'])): ?>
                                    <span class="inline-flex items-center text-[11px] font-medium text-[#b45309]">Belum dibayar</span>
                                <?php endif; ?>
                            </div>

                            
                            <?php if(!in_array($statusKey, ['cancelled', 'completed', 'dibatalkan', 'selesai'])): ?>
                                <?php
                                ...
                                <div class="mt-3 max-w-xs">
                                    <div class="w-full bg-[#f0f0f2] rounded-full h-1" role="progressbar" aria-valuenow="<?php echo e($progressPct); ?>" aria-valuemin="0" aria-valuemax="100">
                                        <div class="h-1 rounded-full bg-gradient-to-r from-[#1d1d1f] to-[#48484a] transition-all duration-500" style="width: <?php echo e($progressPct); ?>%"></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-3.5">
                                <a href="<?php echo e($transaction->detail_link); ?>" class="btn-dark-apple !text-xs !px-4 !py-2">
                                    Detail
                                </a>
                                <a href="https://wa.me/<?php echo e($waPhone); ?>?text=<?php echo e($waText); ?>" target="_blank" rel="noopener"
                                   class="inline-flex items-center gap-1.5 text-xs font-medium px-3.5 py-2 rounded-full border border-[#25D366]/40 text-[#128C4A] hover:bg-[#25D366]/10 transition-colors">
                                ...
                                <?php if($isSewa && $transaction->canBeCancelled()): ?>
                                    <form method="POST" action="<?php echo e(route('customer.transactions.cancel', $transaction->id)); ?>" onsubmit="return confirm('Yakin ingin membatalkan transaksi ini?')">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="text-xs font-medium px-3 py-2 rounded-full text-[#d70015] hover:bg-[#fef2f2] transition-colors">Batal</button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <div class="card-apple-static">
                <div class="p-12 text-center">
                    ...
                    <div class="flex gap-2 justify-center flex-wrap">
                        <a href="<?php echo e(route('customer.products.index')); ?>" class="btn-dark-apple !text-sm !px-5 !py-2.5">Sewa Kamera</a>
                        <a href="<?php echo e(route('customer.studio.index')); ?>" class="btn-outline-apple !text-sm !px-5 !py-2.5">Booking Studio</a>
                    </div>
                </div>
            </div>
        <?php endif; ?>
...
<?php echo $__env->make('layouts.customer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\PROJECT KP\resources\views/customer/transactions/index.blade.php ENDPATH**/ ?>